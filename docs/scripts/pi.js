...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showDoom() {
    restartGif(doomImg);
    doomPopup.classList.add("show");
    setTimeout(() => {
      doomPopup.classList.remove("show");
      setTimeout(showDoom, repeatDelay);
    }, gifDuration);
  }
  ...
  // ✅ Disable right-click on the GIF
  doomImg.addEventListener("contextmenu", function(e) {
    e.preventDefault();
  });

  // ✅ Disable drag-and-drop on the GIF
  doomImg.addEventListener("dragstart", function(e) {
    e.preventDefault();
  });
});
